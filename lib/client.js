...
Client.prototype.debug_eval_in_frame = function(frame, expr, thread, pkg='COMMON-LISP-USER') {
  ...
  _this = this;
  return this.rex(cmd, pkg, thread).then(function(package_) {
    var cmd = '(SWANK:EVAL-STRING-IN-FRAME ' + util.to_lisp_string(expr) + ' '
                                           + frame + ' ' + package_.source + ')';
...
Client.prototype.compile_string = function(compile_string, filename, filename_full, position, line, column, package_) {
  ...
  return this.rex(cmd, package_, "T").then((result) => this.on_compilation(package_, result));
}

// Compiles the given file
Client.prototype.compile_file = function(filename, package_, load=true) {
...
Client.prototype.on_compilation = function(package_, result) {
  ...
  if (result.type == 'list'
    ...
    return this.rex("(SWANK:LOAD-FILE " + result.children[5].source + ")", package_, "T")
  }
}
...
Client.prototype.expand = function(form, pkg, repeatedly=true, macros=true, compiler_macros=true) {
  var func;
  if (macros) {
    if (compiler_macros) {
      func = "EXPAND";
  ...
  return this.rex(cmd, pkg, "T").then(util.from_lisp_string);
}
